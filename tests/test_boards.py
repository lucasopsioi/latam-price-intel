...
ACME = db.q1("SELECT id FROM brand WHERE name='Acme'")["id"]
    ...
    with db.tx() as c:
        for i in range(n):
            ...
            c.execute("""INSERT INTO price_obs(obs_date,country_code,channel_id,brand_id,
                         ...
                         VALUES(?,'MX',?,?,?,?,?,'MXN',?,'accepted',1,?,?,'new',0,?,?,?,?)""",
...
add("2020-01-05", 900, ACME, 300, kind="unknown", n=8)     # Acme"屏幕"，300 块
...
add("2020-01-05", 901, ACME, 300, kind="device", n=8)
...
with db.tx() as c:
    ...
    for day, seller, price, ram, rom in [
        ...
        c.execute("""INSERT INTO price_obs(obs_date,country_code,channel_id,brand_id,
                     ...
                     VALUES(?,'MX',900,900,'pc','TestBook',?,'MXN',?,'accepted',1,?,
                            'device','new',0,?,?,?,900)""",
