...
# ★★ 2026-09-07 换机制：脚本不再走 -EncodedCommand，改写 .ps1 文件用 -File。
#   EncodedCommand 是命令行参数，受 Windows 32,767 字符上限约束，而脚本长度随
#   待同步文件数线性增长 —— 实测 20 个文件 = 39,289 字符脚本 / 104,770 字符编码后，
#   必然 `WinError 206 文件名或扩展名太长`。日志里这条错误出现 **2,232 次**，
#   手机自动同步一直是坏的，而 APScheduler 吞掉异常后照打 "executed successfully"，
#   于是「同步失败」与「没有新文件」在界面上长得一模一样。
# ★ 用 ast 不用字符串搜索：注释里就写着 "-EncodedCommand"（说明为什么不用它），
#   裸 `"-EncodedCommand" not in src` 会被自己的注释咬住 —— 这正是
#   knowledge/lessons/assertions-that-verify-nothing 记的那一课。
...
for _n in _ast.walk(_tree):
    if (isinstance(_n, _ast.Call) and isinstance(_n.func, _ast.Attribute)
            ...
            and isinstance(_n.args[0], (_ast.List, _ast.Tuple))):
        _run_args.append([e.value for e in _n.args[0].elts
                          if isinstance(e, _ast.Constant) and isinstance(e.value, str)])
_ps = [a for a in _run_args if a and "powershell" in a[0].lower()]
ok(bool(_ps), "找到 PowerShell 子进程调用")
ok(all("-EncodedCommand" not in a and "-File" in a for a in _ps),
   "★ 必须走 .ps1 文件（-File），不许用 -EncodedCommand —— 后者有 32,767 字符"
   f"命令行上限，文件一多必然 WinError 206（已实测炸过 2,232 次）。实得 {_ps}")
ok("utf-8-sig" in src,
   "★ 中文 .ps1 必须 UTF-8 **带 BOM** —— 本机 ANSI=cp936，无 BOM 时 PowerShell "
   "按 cp936 解码，中文日志变乱码（knowledge/lessons/windows-cp936-encoding）")
ok('newline="\\r\\n"' in src or "newline='\\r\\n'" in src,
   "★ .ps1 用 CRLF 换行 —— PowerShell 对纯 LF 的多行脚本在个别版本上解析异常")
ok("ExecutionPolicy" in src and "Bypass" in src,
   "★ -File 跑 .ps1 会受执行策略约束，必须显式 Bypass，否则静默不执行")
