...
print("== 2026-09-04 现行犯：五层叠加 + 2 字残片「a Previa」+ Ripley 品牌芯片评分 ==")
# 假产品 #9204「A Previa Tarjetazo Cupon」就是这条标题建出来的（obs 218761，969,990 CLP）
check("★ a Previa + TARJETAZO + CUPÓN ACER10 + 4 cuotas + ACER 5.0 五层全剥",
      strip_ui_chrome("a Previa TARJETAZO CUPÓN ACER10 4 cuotas sin interés ACER 5.0 NOTEBOOK ACER NITRO LITE 16 NL16-71G-50GH-CL"),
      "NOTEBOOK ACER NITRO LITE 16 NL16-71G-50GH-CL")
check("残片 ista Previa", strip_ui_chrome("ista Previa Apple iPad Air 11"), "Apple iPad Air 11")
# Ripley 商品卡的「品牌芯片 + 评分」（60d 2,890 条，假型号「5.0 Vivo Y19S」60 条观测）
check("品牌在商品名里再出现 → 整个芯片剥掉",
      strip_ui_chrome("ACER 5.0 NOTEBOOK ACER NITRO LITE 16"), "NOTEBOOK ACER NITRO LITE 16")
check("品牌只出现一次 → 只剥评分、保住品牌词（guess_brand 靠它）",
      strip_ui_chrome("APPLE 4.7 AIRPODS PRO 3"), "APPLE AIRPODS PRO 3")
check("SONY 4.5 Audífonos → SONY Audífonos", strip_ui_chrome("SONY 4.5 Audífonos WH-CH520 Bluetooth"),
      "SONY Audífonos WH-CH520 Bluetooth")
check("GENÉRICO 芯片（带重音的大写）", strip_ui_chrome("GENÉRICO 5.0 CARGADOR MAGNÉTICO SAMSUNG GALAXY WATCH 4"),
      "GENÉRICO CARGADOR MAGNÉTICO SAMSUNG GALAXY WATCH 4")
check("芯片剥后再叠成色词尾巴：condition 由 detect_condition 另算",
      strip_ui_chrome("APPLE 3.0 IPHONE 13 128 GB ROSADO - REACONDICIONADO"),
      "APPLE IPHONE 13 128 GB ROSADO - REACONDICIONADO")
# ★ 反向：首字母大写的真规格、声道数、单位 —— 一个字不动
unchanged("Bluetooth 5.4 是规格（非全大写芯片）", "Bluetooth 5.4 con ANC - Cancelación Activa de Ruido")
unchanged("Audífonos 3.5 mm 是规格", "Audífonos 3.5 mm con Micrófono – Sonido Estéreo")
unchanged("LG 2.1 CH 是声道数", "LG 2.1 CH SOUNDBAR")
unchanged("SAMSUNG 5.1 SOUNDBAR 声道数", "SAMSUNG 5.1 SOUNDBAR ACME-Q600")
unchanged("SLATETAB 11.5 评分位 >5", "SLATETAB 11.5 128GB")
# ★ 成色词：只有后面还跟着别的角标时才当角标剥；直接跟商品名的保留（成色信息别丢）
unchanged("行首 Reacondicionado 直接跟商品名 → 保留", "Reacondicionado APPLE iPhone 13 128GB")
check("行首 Reacondicionado + 分期角标 → 叠层，剥",
      strip_ui_chrome("Reacondicionado 4 cuotas sin interés VIVO Y29"), "VIVO Y29")
check("阴性 Reacondicionada + 分期角标 → 叠层，剥",
      strip_ui_chrome("Reacondicionada 4 cuotas sin interés APPLE iPhone 12"), "APPLE iPhone 12")
for _s in ("a Previa TARJETAZO CUPÓN ACER10 4 cuotas sin interés ACER 5.0 NOTEBOOK ACER NITRO LITE 16",
           "APPLE 4.7 AIRPODS PRO 3", "Reacondicionado APPLE iPhone 13 128GB"):
    _once = strip_ui_chrome(_s)
    check(f"幂等 {_s[:38]!r}…", strip_ui_chrome(_once), _once)
