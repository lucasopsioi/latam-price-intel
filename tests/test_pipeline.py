...
for name in ("iOS 27", "iPadOS 27", "macOS 27", "watchOS 27", "tvOS 26",
             ...
             "One UI 8", "AcmeOS 6", "HyperOS 3", "Wear OS 6", "ColorOS 16"):
...
# ─────────── 抓取口径四决策（2026-08-28 用户拍板，见 knowledge/log.md） ───────────
# 用户原话：「检查一下抓取价格的部分有没有我没讲清楚的需求或者是应该要
# 敲定的逻辑」—— 审计出四个从未拍板的口径，逐一定死：
from app.scraping.extract import detect_price_scope  # noqa: E402

# ① 现价口径 = 公开价优先：卡专享价识别（只认专享语义，裸 tarjeta 不误伤）
for t, want in [
    ("Vista Previa TARJETA FEST GARMIN SMARTWATCH", "card"),
    ("HOT PRICE 10% adicional tarjeta SAMSUNG Galaxy Book", "card"),
    ("Precio tarjeta CMR Falabella iPhone 16e", "card"),
    ("Paga con tarjeta o PIX - iPhone 16e 128GB", "public"),   # 支付方式≠专享
    ("Smartphone Samsung Galaxy S25 256GB", "public"),
]:
    check_true(f"★ 卡价判定 {want}：{t[:44]}", detect_price_scope(t) == want)

_pm = (ROOT / "app/agents/pricemove.py").read_text(encoding="utf-8")
# ② 同日多价 = 自营价优先（cur 与 prev 两侧都要，只改一侧会比出假变动）
check_true("★ 比价两侧（cur_agg 与 prev_agg）都必须自营价优先 —— 只改一侧，"    "自营价和第三方最低价互比就是假变动", _pm.count("'self_operated','brand_official'") >= 2)
check_true("★ 卡专享价不进比价（两侧都要挡）", _pm.count("<> 'card'") >= 2)

# ③ 采集节奏 = 每日全品类（回退开关保留）
_or = (ROOT / "app/agents/orchestrator.py").read_text(encoding="utf-8")
check_true("★ 每日全品类要有配置开关（scrape.daily_all_categories，0 可回退轮换）", 'daily_all_categories' in _or)
import yaml as _yaml  # noqa: E402
_rt = _yaml.safe_load((ROOT / "config/runtime.yaml").read_text(encoding="utf-8"))
check_true("★ 配置里每日全品类必须开着（用户拍板）", int((_rt.get("scrape") or {}).get("daily_all_categories", 0)) == 1)

# ①的采集端接线 + 周报侧过滤
_bs = (ROOT / "app/scraping/channels/base.py").read_text(encoding="utf-8")
check_true("采集端要在列表页判定价格口径", "detect_price_scope" in _bs)
_wk2 = (ROOT / "app/agents/weekly.py").read_text(encoding="utf-8")
check_true("周报的产品曲线要挡卡专享价", "<> 'card'" in _wk2)
check_true("产品曲线同日多卖家时自营价优先", _wk2.count("'self_operated','brand_official'") >= 1)
