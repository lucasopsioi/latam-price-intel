...
# ─────────── 渠道内部货号绝不能当型号名（2026-08-28 用户点名） ───────────
# 用户看到巴西穿戴表里全是 "MRPBAB0024PTO_PRD" 这类看不出机型的名字。
# 根因：cleaner._authoritative_sku 无条件信任 price_obs.sku_code，
# 而 Fast Shop（VTEX）把渠道内部商品号写进了这个字段 ⇒
# 503 个巴西产品里 492 个（98%）的型号名变成了货号，
# 「iPhone 17 Apple (256GB)」显示成「AEMG6L4BRAAZL_PRD」。
# 这是同一病根**第二次发作**（第一次是 Alkosto 把 EAN 条形码写进 sku_code），
# 说明"拒绝已知坏形态"是打地鼠 —— 必须改成权威表白名单。
from app.agents.cleaner import _authoritative_sku   # noqa: E402

for junk_code in ("221064_Fastshopbr", "AEMG6L4BRAAZL_PRD", "195949776212",
                  "80560111010035402", "JBLT530BTPTO_PRD"):
    ok(_authoritative_sku(junk_code) == "",
       f"★ 渠道内部货号不能当型号名：{junk_code}")
for real in ("Apple iPad Air 11 M3", "Samsung Galaxy Tab S10 Lite"):
    ok(_authoritative_sku(real) == real,
       f"权威 SKU_Short 值必须照常认：{real}")

_cl = (ROOT / "app/agents/cleaner.py").read_text(encoding="utf-8")
ok("_sku_whitelist" in _cl and "skumap" in _cl,
   "★ 必须用权威表**白名单**判定，不能再靠拒绝已知坏形态（打地鼠）")

# 配置要能解析出来：同一机型不同存储是不同价位段
from app.scraping.extract import parse_ram_rom      # noqa: E402

ok(parse_ram_rom("Motorola Edge 70 Fusion Plus512GB 5G")[1] == 512,
   "★ 容量紧贴词尾（Plus512GB）也要解析出来 —— \b 边界不成立会整条漏掉")
ok(parse_ram_rom("Xiaomi Redmi 15C 5G BR 8GB RAM 256GB") == (8, 256),
   "正常写法的 RAM/ROM 不能被改坏")

from app.agents.weekly import _label, _cfg_tag      # noqa: E402

ok(_label("Honor", "Honor 70 Honor") == "Honor 70",
   "★ 型号里重复出现的品牌词要去掉（实测 66 个 'Honor Honor 70 Honor'）")
ok(_cfg_tag({"rom_gb": 256, "ram_gb": 8}) == " 8+256GB", "配置尾巴：内存+存储")
ok(_cfg_tag({"rom_gb": 1024}) == " 1TB", "1024GB 要写成 1TB")
ok(_cfg_tag({"rom_gb": None}) == "", "解析不出配置就留空，不猜")
