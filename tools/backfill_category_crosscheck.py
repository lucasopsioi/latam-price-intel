...
def scan(only_cat: str | None = None) -> tuple[list[dict], int]:
    rows = db.q("""
        ...
        -- ★ unknown 也要扫：Moto Tag 防丢器 31 行全是 product_kind='unknown'，
        --   只扫 device 会漏掉（accessory 不扫：kind 判对了自然不进设备分析）
        WHERE po.product_kind IN ('device', 'unknown')
    ...
    for r in rows:
        if only_cat and r["category_code"] != only_cat:
            continue
        verdict, target, why = extract.crosscheck_category(
            r["title"], r["category_code"], brand=r["brand"] or "")
        if verdict == "ok":
            continue
        d = dict(r)
