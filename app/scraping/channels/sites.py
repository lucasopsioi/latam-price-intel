...
_ACME_CONTAINERS = ("productList", "flashSaleProductList", "singleProductList",
                  "tabProductList")


def _acme_arrays(raw: str, key: str):
    """把 "key":[ ... ] 整段按括号配平取出来。
...
_ACME_BUNDLE = re.compile(r"[A-Za-z]{3,}[^+]{0,24}\+\s*[A-Za-z]{3,}")


def _acme_num(*vals) -> float | None:
...
class AcmeStoreAdapter(BrandStoreAdapter):
    ...
    def parse_listings(self, html: str, text: str) -> list[Listing]:
        ...
        out: list[Listing] = []
        seen: set = set()
        for key in _ACME_CONTAINERS:
            for arr in _acme_arrays(raw, key):
                try:
    ...
    def _from_json(self, it, seen: set) -> Listing | None:
        ...
        #   是两套口径，前者一律直读 float。
        sale = _acme_num(it.get("salePrice"), it.get("unitPrice"), it.get("currentPrice"))
        listp = _acme_num(it.get("streetPrice"), it.get("specialPrice"), it.get("orderPrice"))
        ...
        if lst.product_kind == "unknown":
            ...
            if _ACME_ACCESSORY.search(lst.title):
        ...
        if _ACME_BUNDLE.search(lst.title):
            lst.is_bundle = True
