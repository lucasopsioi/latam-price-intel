    ...
    def _persist(self, items: list[Listing], channel: dict, country: dict,
        ...
        with db.tx() as conn:
            for it in items:
                ...
                # ★ 品类交叉校验（2026-08-28 接入采集端）：category 是「在抓哪个
                #   品类页」，不是商品本身 —— 穿戴搜索页会混出手机/耳机
                #   （实测 8859 行错分：OPPO A5 手机进穿戴、JBL 耳机进穿戴、
                #   Moto Tag 防丢器进穿戴）。标题证据明确指向他类就改判，
                #   指向多类或是防丢器/玩具就置 NULL（不进任何品类分析）；
                #   标题无证据时用品牌品类先验（brands.yaml，用户口径）。
                #   规则与历史回填工具共用同一份实现（extract.crosscheck_category），
                #   两套规则的差集永远不会有人发现 —— 所以必须是同一份。
                from . import extract as _ex
                verdict, target, _why = _ex.crosscheck_category(
                    it.title, category, brand=bname or brand["name"])
                cat_eff = (target if verdict == "fix"
                           else None if verdict == "pending" else category)
                ...
                try:
                    cur = conn.execute("""
                        INSERT OR IGNORE INTO price_obs(
                          ...
                          url,row_hash,run_id,audit_reason,price_scope)
                        VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)
                    """, (obs_date, country["code"], channel["id"], brand_id, cat_eff,
