...
class GsmArenaSource:
    ...
    def __init__(self, delay: float = 6.0, timeout: float = 30.0, engine=None):
        ...
        # 限流自适应：连续挨打计数 + 收工开关（见 _get 里的说明）
        self._streak = 0
        self.max_block_streak = 4        # 60+120+240+480 秒都没缓过来 → 今天到额
        self.give_up = False
    ...
    def _get(self, url: str) -> str | None:
        ...
        if r.status_code == 429 or r.status_code == 503:
            # ★★ 2026-09-03 实测事故：固定退避 60 秒 + 无限重试 =
            #   撞了 195 次 429（白等 3 小时还在继续压），把 IP 级限流坐实。
            #   限流不是"等一下就好"的抖动，是站方在告诉我们**今天到额了**。
            # ⇒ 指数退避（60→120→240…封顶 15 分钟）+ 连续挨打就收工。
            #   宁可今晚少补几百个，也不要把 IP 打进更长的黑名单。
            self.stats["blocked"] += 1
            self._streak += 1
            if self._streak >= self.max_block_streak:
                log.error("[gsmarena] 连续 %d 次被限流，本轮收工（下轮再续，"
                          "已取到的规格都已落库）", self._streak)
                self.give_up = True
                return None
            wait = min(60 * (2 ** (self._streak - 1)), 900)
            log.warning("[gsmarena] 被限流 HTTP %s（连续第 %d 次），退避 %d 秒",
                        r.status_code, self._streak, wait)
            time.sleep(wait)
            return None
        self._streak = 0                    # 成功一次就重置连击计数
