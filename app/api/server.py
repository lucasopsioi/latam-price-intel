...
SECRET_KEYS = {
    "minimax_api_key", "deepseek_api_key", "meli_access_token", "x_bearer_token",
    "telegram_bot_token", "openai_api_key", "proxy_password",
}
PLAIN_KEYS = {
    "llm_base_url", "llm_model", "llm_model_deepseek", "proxy", "telegram_chat_id",
}
...
def test_settings():
    ...
    # ①b DeepSeek —— 出报告的写作模型（2026-08-28 起）
    dkey = db.get_setting("deepseek_api_key")
    if not dkey:
        results.append({"name": "DeepSeek", "ok": False,
                        "message": "未配置（报告将回落 MiniMax 生成）"})
    else:
        try:
            from ..agents import LLMClient
            cfg = config.load_runtime()["agents"]
            dllm = LLMClient(cfg, provider="deepseek")
            reply, tokens = dllm.chat("回复两个字：正常")
            ok = bool(reply and reply.strip())
            results.append({"name": "DeepSeek", "ok": ok,
                            "message": (f"连通，模型 {dllm.model} 返回"
                                        f"「{reply.strip()[:12]}」（{tokens} tokens）") if ok
                                       else f"调用无返回 —— 检查模型名（当前 {dllm.model}）与余额"})
        except Exception as e:                          # noqa: BLE001
            results.append({"name": "DeepSeek", "ok": False,
...
def list_models(provider: str = "deepseek"):
    """列出该供应商账号下**真实可用**的模型，给设置页的下拉框用。

    ★ 不写死候选列表：模型会上下线，写死的选项迟早对不上账号实际权限
      （用户要选 V4 Pro，而 pro/flash 各账号开通情况不同）。
      直接问供应商的 /models 接口，拿到什么给什么。
    ★ 取不到就返回空 list + reason，前端退回可手填的输入框 ——
      不能因为列不出模型就卡住用户配置。
    """
    ...
    if not pv:
        return {"models": [], "reason": f"未知供应商 {provider}"}
    key = db.get_setting(pv["key_setting"], "")
    if not key:
        return {"models": [], "reason": "尚未配置该供应商的 Key"}
    base = (db.get_setting(pv["base_setting"]) or pv["base"]).rstrip("/")
    try:
        import httpx
        r = httpx.get(f"{base}/models", timeout=20,
                      headers={"Authorization": f"Bearer {key}"})
        if r.status_code != 200:
            return {"models": [], "reason": f"接口返回 HTTP {r.status_code}"}
        ids = [m.get("id") for m in (r.json().get("data") or []) if m.get("id")]
        return {"models": sorted(ids), "current": db.get_setting(pv["model_setting"])
                or pv["model_default"]}
    except Exception as e:                              # noqa: BLE001
        return {"models": [], "reason": f"取模型列表失败：{str(e)[:80]}"}
...
@app.get("/api/products/grid/{kind}")
def grid_get(kind: str):
    """在线表格取数：kind = product | sku | pricing"""
    try:
        return products.grid_rows(kind)
    except ValueError as e:
        raise HTTPException(400, str(e))


@app.post("/api/products/grid/{kind}")
def grid_post(kind: str, payload: dict):
    """在线表格保存。payload = {changes: [{key, fields}]}

    ★ 可写列由 products.GRID_TABLES 白名单锁死 —— 前端传什么都越不了界。
    """
    try:
        r = products.grid_save(kind, payload.get("changes") or [])
    except ValueError as e:
        raise HTTPException(400, str(e))
    return r
...
def trend_candidates(limit: int = 60):
    ...
    # ★ 只数审计通过的观测（2026-09-04 切换）：pending 是"还没审"不是"审过了没问题"，
    #   拿它算 obs_days 会把只有待审观测的品牌推进下拉，点开却画不出线。
    brands = db.q("""SELECT b.name, b.is_ours,
                     ...
                     WHERE po.audit_status='accepted'
...
def schedule_status():
    """定时任务状态：下次自动采集 / 自动出周报 / 手机同步是什么时候。

    ★ 用户要能自己确认「这软件是不是真的会自动干活」，
      而不是只能听人说会。
    """
    from .. import scheduler
    st = scheduler.status()
    st["llm"] = {
        # 搜索/研判类 Agent 用 MiniMax；出报告用 DeepSeek（未配 Key 则回落）
        "minimax": {"configured": bool(db.get_setting("minimax_api_key")),
                    "model": db.get_setting("llm_model") or "MiniMax-Text-01"},
        "report": {
            "provider": "deepseek" if db.get_setting("deepseek_api_key") else "minimax（回落，DeepSeek Key 未配）",
            "model": (db.get_setting("llm_model_deepseek") or "deepseek-chat")
                     if db.get_setting("deepseek_api_key")
                     else (db.get_setting("llm_model") or "MiniMax-Text-01"),
        },
    }
    last = db.q1("""SELECT id, title, week_start, week_end, created_at
                    FROM weekly_report ORDER BY id DESC LIMIT 1""")
    st["last_report"] = last
    return st
...
# ★★ 任务槽的最长占用时长（2026-09-07 加）。
#   正常一轮采集 17~19 小时（实测 #54=19h、#55=17h），所以阈值必须明显大于它，
#   否则会把还在正常干活的采集判成僵死。取 30 小时。
#   为什么需要它：2026-09-04 12:30 那轮采集**挂住没结束**，而任务槽是**进程内
#   的内存标志**，只有服务重启才会清 —— 于是 09-05、09-06 两天的定时采集全部
#   被「已有任务在运行，本次跳过」挡掉，**连续三天没有新数据**，而界面上
#   `task_running: true` 看起来一切正常。加上 09-07 服务死在 07:30 之前，
#   一共丢了四天。这是「卡死」与「正常在跑」长得一样的又一形态。
TASK_SLOT_MAX_HOURS = 30


def acquire_task_slot(name: str) -> bool:
    """占用单任务槽。定时任务与界面共用同一把闸 —— 见 scheduler._job_collect。

    ★ 超过 TASK_SLOT_MAX_HOURS 仍未释放的槽视为僵死，强行接管并**留痕告警**：
      静默接管等于把"采集卡死"这件事也一并抹掉，下次还会发生且还是没人知道。
    """
    with _TASK_LOCK:
        if _TASK["running"]:
            held = time.time() - (_TASK.get("started_at") or time.time())
            if held < TASK_SLOT_MAX_HOURS * 3600:
                return False
            log.error("★ 任务槽被「%s」占用 %.1f 小时（超过 %d 小时上限），判为僵死并接管。"
                      "上一轮任务很可能挂在网络/浏览器上没退出 —— 请查 scrape_run "
                      "最后一批的 status 与 logs/run-*.log 的尾部。",
                      _TASK.get("name"), held / 3600, TASK_SLOT_MAX_HOURS)
            try:                                   # 把上一批标成 interrupted，别留 running 悬着
                # ★ 必须走 db.tx()：db.q1 只 execute 不 commit，UPDATE 会静默丢失
                with db.tx() as _c:
                    _c.execute("UPDATE scrape_run SET status='interrupted', "
                               "finished_at=datetime('now') WHERE status='running'")
            except Exception:                      # noqa: BLE001
                log.warning("接管任务槽时收尾 scrape_run 失败", exc_info=True)
    ...
    def worker():
        ...
        try:
            if action == "collect":
                ...
                cfg = config.load_runtime()["agents"]
                # ★ 手动审计：昨天 + 今天各审到取尽（跨午夜批次会横跨两天）。
                #   旧写法 .run() 只审今天前 2000 行，且名额按国家字母序分配。
                ag = PriceAuditAgent(LLMClient(cfg), cfg)
                out: dict = {}
                for d in ((_date.today() - _td(days=1)).isoformat(), _date.today().isoformat()):
                    _TASK["progress"] = f"审计 {d}…"
                    r = ag.run_all(obs_date=d) or {}
                    for k, v in r.items():
                        if isinstance(v, (int, float)):
                            out[k] = out.get(k, 0) + v
                    if r.get("warning"):
                        out["warning"] = (out.get("warning", "") + f"[{d}] {r['warning']} ").strip()
                _TASK["result"] = out
            elif action == "match":
                from ..matching import CompetitorMatcher
                ...
                # ★ 与周一定时任务**同一条链路**（scheduler.generate_weekly）：
                #   MiniMax 生成 → 导出 PDF/Word/PPT → 推手机。
                #   界面点一次和定时跑一次，行为必须完全一致，
                #   否则"自动出的报告"和"手点出的报告"会长得不一样。
                from .. import scheduler as _sched
                _TASK["progress"] = "调用 MiniMax 生成周报中…"
                _pl = payload or {}
                _TASK["result"] = _sched.generate_weekly(
                    anchor=_pl.get("period") or None,
