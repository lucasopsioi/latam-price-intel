...
def spec_similarity(mine: dict, rival: dict, category: str) -> dict:
    ...
    for key, w in weights.items():
        # ★ extra 是"该品类还没有标准化数值维度"的占位（音频 1.0、穿戴 0.4），
        #   它**永远比不了**，所以不参与打分 —— 但必须计入 total_w。
        #   原来写在 total_w += w 之前 continue，等于把"比不了的那部分权重"
        #   从分母里抹掉：穿戴只比了 screen/battery（占真实权重 60%）却报
        #   confidence=1.00，"我只看了六成"被说成"我全看过了"。
        #   confidence 是给人看的可信度标签，虚报比不报更糟。
        total_w += w
        if key == "extra":          # 无标准化数值规格，计入分母但不打分
            missing.append(key)
            continue
