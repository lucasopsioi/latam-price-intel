...
class CompetitorMatcher:
    ...
    def rebuild_all(self, my_product_id: int | None = None) -> dict:
        ...
        # ★ 我方报价索引是**类属性**缓存，而服务是常驻的（计划任务 →
        #   supervisor → serve，进程能活好几天）。不在每轮开头清一次，
        #   走型号键兜底路径取价的产品（约 18% 的产品靠它，命中率 48%→66%）
        #   我方价会永远冻结在进程启动后第一次 rebuild 时的值 ——
        #   商城调了价，价差看板照样拿旧价算，且完全无声。
        CompetitorMatcher._OUR_INDEX.clear()

        # ★ 只算**启用品类**的产品。category 表里 other/accessory 是
        #   enabled=0 的"禁用品类"。品类判定每天在跑，万一某款我方产品被
        #   判进 other，_candidates 会拿 other 下几百个电视/音箱/投影当候选，
        #   产出一份纯价格带的垃圾榜单 —— 而它长得和正常榜单一模一样。
        where = ("WHERE mp.status='active' "
                 "AND EXISTS(SELECT 1 FROM category ct "
                 "           WHERE ct.code=mp.category_code AND ct.enabled=1)")
        params: list = []
        if my_product_id:
            where += " AND mp.id=?"
        ...
        # ★ 收尾自检：跨品类不变量。
        #   这类污染**不会自己暴露** —— 脏行不报错、不变色、computed_at 也不动，
        #   界面上和正常匹配长得一模一样，只会等用户截图（2026-09-03
        #   「SonicBuds 5 的竞品是小米手表」就是这么被发现的）。
        #   候选池 SQL 有 rp.category_code=? 硬约束，所以本引擎自己**不可能**
        #   产出跨品类行；一旦这里 >0，说明有**外部工具**改了 rival_product
        #   的品类或改指了 rival_product_id 而没重算 —— 报出来，别让它沉默。
        #   只查用户看得见的行（is_excluded=0）：被人工排除的历史脏行无害，
        #   删掉反而丢了"用户说过这不是竞品"这个判断。
        cross = db.q1("""
        ...
        warn = ""
        if cross:
            warn = (f"⚠ 重算后仍有 {cross} 条**跨品类**匹配（我方与友商品类不同）。"
                    f"本引擎不可能产出这种行 —— 一定是外部工具改了友商品类或"
                    f"改指了 rival_product_id 却没重算，请查 reclass_audit")
            log.warning(warn)
    ...
    def _match_one(self, mp: dict, country: dict) -> int:
        ...
        if not mine_price or not mine_price["rrp_local"]:
            # ★ 取不到我方价必须**先把旧结论作废再退出**，不能直接 return。
            #   原来的写法在 _persist 之前早退 ⇒ 清理用的 DELETE 从不执行 ⇒
            #   上一轮的匹配连同当时的价格快照原地冻结，computed_at 也不更新，
            #   界面上和新算的一模一样。实测 Vega 80/CO 的 4 条匹配停在
            #   2026-08-13，而同一款的 CL/PE 是 09-03 的新数 —— 越是这种
            #   "只有一个国家旧"的情况越没人能发现。
            #   "算不出来"要落成"没有结论"，不是"沿用上次的结论"。
            self._persist(mp["id"], cc, [])
            return 0
    ...
    def _candidates(self, category: str, cc: str, currency: str) -> list[dict]:
        ...
        return db.q("""
            ...
            JOIN category ct    ON ct.code = rp.category_code AND ct.enabled = 1
            ...
            WHERE rp.category_code = ?
              ...
              AND po.is_bundle     = 0
              -- ★ 口径要**两侧对称**。_our_price 那边正面断言 device 把配件
              --   挡在外面，这一侧却什么都不挡 ⇒ 保护壳、表带、副厂屏幕的价
              --   会被 AVG 平进"友商价"，再拿去和我方整机价比。
              --   这里只排掉**已明确判定为配件**的观测（不排 unknown ——
              --   unknown 里大量是还没判的整机，正面断言 device 会一次砍掉
              --   29% 的候选，那是把"还没处理"当成"不合格"，与上面 audit_status
              --   踩过的是同一个坑）。实测：五个分析品类的候选产品数一个不减，
              --   只修正了 8 条匹配被配件观测拉偏的均价。
              AND COALESCE(po.product_kind,'') <> 'accessory'
              -- ★ 桶不进候选池。"Lenovo Tab"/"Samsung Galaxy Tab" 这类系列级名字
              --   下面挂着几十款不同价位的机器，AVG(sale_price) 算出来的"友商价"
              --   不属于任何一款真实产品，拿去和我方整机比规格比价全是噪声；
              --   而且它的 obs_count 天然最大，会稳定挤进 top_n。
              --   桶的判据与标记：app/skunorm.py bucket_verdict、tools/mark_buckets.py。
