...
(function (global) {
  ...
  function ask(question, elId, o) {
    ...
    // 而跨币种的中位数比的是币种不是价格。
    // 调用方给了 currencies 列表却没标 mixedCurrency 的，按列表判 —— 少一处漏判。
    var mixed = !!o.mixedCurrency || !!(o.currencies && o.currencies.length > 1);
    if (question === 'change' && mixed && !o.indexed) {
      return empty(elId, {
        title: '跨币种不能比绝对价位',
  ...
  /**
   * 把 /api/trend/series（trends.product_series / category_series / brand_series）
   * 的返回装成 ask('change') 的参数 —— **币种口径只写这一处**。
   *
   * ★ 规则：多币种（r.mixed_currency）必须走指数；要走指数而某条线没有 index
   *   就不画（blocked + 原因），**绝不退回绝对价** —— 退回去画出来的图纵轴是
   *   第一条线的币种、其余线全错，而且不报错（实测 #2292 Lenovo Tab 就是这么错的：
   *   以前 product_series 不返回 index，`showIndex = 勾选 && s0.index` 恒为假）。
   * ★ 纵轴标签：单币种标该币种；多币种只能是「指数（基期=100）」。
   *
   * 用法：var t = Charts.trendSeries(r, wantIndex);
   *       t.blocked ? Charts.empty(el, t) : Charts.ask('change', el, {xs:r.xs, series:t.series,
   *         ylab:t.ylab, fmt:t.fmt, indexed:t.indexed, mixedCurrency:t.mixedCurrency, ...});
   */
  function trendSeries(r, wantIndex) {
    r = r || {};
    var all = r.series || [];
    var indexed = !!(wantIndex || r.mixed_currency || r.indexed_required);
    if (indexed) {
      var lacking = all.filter(function (s) {
        return !s.index || !s.index.some(function (v) { return v != null; });
      });
      if (lacking.length) {
        return {
          series: [], indexed: true, blocked: true,
          title: r.mixed_currency ? '跨币种只能画指数，但缺少指数序列' : '缺少指数序列',
          reason: (r.mixed_currency
            ? '横跨 ' + ((r.currencies || []).join('/') || '多个') + ' 个币种，绝对价不可同轴；' : '')
            + lacking.length + ' 条线没有 index（'
            + lacking.map(function (s) { return s.name; }).slice(0, 4).join('、') + '）',
          action: '后端应对每条线给出 index（首个真实观测=100）；在此之前不画绝对价 —— '
                + '那张图的纵轴会是第一条线的币种，其余线全错',
        };
      }
    }
    var series = all.map(function (s) {
      return { name: s.name, filled: s.filled, n: s.n, pts: indexed ? s.index : s.pts };
    });
    var cur = (r.currencies && r.currencies.length === 1) ? r.currencies[0]
      : ((all[0] || {}).currency || '本币');
    return {
      series: series, indexed: indexed, blocked: false,
      ylab: indexed ? '指数（基期=100）' : (r.y_label || ('价格（' + cur + '）')),
      fmt: indexed
        ? function (v) { return v == null ? '—' : Number(v).toFixed(1); }
        : function (v) { return v == null ? '未采集' : Math.round(v).toLocaleString(); },
      mixedCurrency: !!r.mixed_currency, currencies: r.currencies || [],
    };
  }
  ...
  global.Charts = {
    ...
    // 趋势接口返回 → ask('change') 参数（币种/指数口径的唯一实现）
