...
async function loadTrendBoard() {
  ...
  // ★ 展示名优先用后端 label（trends.display_label 去重），缺失才本地 prodLabel ——
  //   裸拼 brand + model_name 会得到「Lenovo Lenovo Tab」：权威 SKU 的 model_name
  //   自带品牌前缀，而入库层不许剥（跨品牌撞车的守卫靠它），去重只能放展示层。
  fill('#tc-product', d.products.map(p => ({
    v: p.id, t: `${p.label || prodLabel(p.brand, p.model_name)}（${p.cat_zh}·${p.obs_days}天）` })));
 ...
 *
 *  ★ 币种与指数口径只在 Charts.trendSeries 里写一处：多币种必走指数；要走指数而某条线
 *    缺 index 就 blocked（画空态、说原因），**绝不退回绝对价**。
 *    以前这里是 `showIndex = 勾选 && s0.index`：product_series 不返回 index 时它恒为假，
 *    多币种产品照样画绝对价、纵轴取第一条线的币种 —— 用户截图的 #2292 Lenovo Tab
 *    在 18 万与 8,500 CLP 之间来回跳，就是这条分支画出来的。
 *  ★ 空图必须分得清「待审计」与「没数据」：接口给了 pending_obs / dropped_series，如实写出。
 *  ★ 所有分支都要刷 #tc-meta / #tc-basket —— 早退会把上一个对象的篮子与说明留在页面上。 */
async function drawSingle() {
  ...
  // 拨了不说等于骗人，不拨就只能给一张空图。
  const forced = !!(r.mixed_currency || r.indexed_required) && !$('#tc-index').checked;
  if (forced) $('#tc-index').checked = true;
  const t = Charts.trendSeries(r, $('#tc-index').checked);
  const pending = Number(r.pending_obs || 0);

  if (t.blocked) {
    Charts.empty('curve-chart', t);
  } else if (!t.series.some(s => s.pts.some(v => v != null))) {
    Charts.empty('curve-chart', {
      title: pending ? '审计通过的观测还画不出曲线' : '这个对象还画不出曲线',
      reason: r.insufficient
        ? `固定篮子只有 ${r.basket} 件商品，不足以代表整体`
        : pending
          ? `本产品另有 ${pending} 条观测待价格审计、未上图（只画审计通过的）；已通过的不足以成线`
          : '所选区间内没有足够的连续观测',
      action: pending
        ? '这不是没数据 —— 审计积压清完会自动补上；也可以换个对象或拉长区间'
        : '换个对象或拉长区间；曲线要连续采集几天才成形',
    });
  } else {
    Charts.ask('change', 'curve-chart', {
      xs: r.xs, series: t.series, ylab: t.ylab, fmt: t.fmt,
      indexed: t.indexed, mixedCurrency: t.mixedCurrency, currencies: t.currencies,
      area: t.series.length === 1,
    });
  }
  renderCurveMeta(r, forced, pending);
  renderCurveBasket(r);
}

/** 篮子/断点/币种/待审计/未画的线 —— 这些决定这条线可不可信，必须摆出来 */
function renderCurveMeta(r, forced, pending) {
  const meta = [];
  ...
  if (r.mixed_currency) {
    meta.push(`⚠ 横跨 ${(r.currencies || []).join('/')} —— `
      + `绝对价位已停用${forced ? '，已自动切到指数口径' : ''}。`
      + `跨币种的中位数比的是币种不是价格：中位那件今天落在秘鲁索尔、`
      + `明天落在墨西哥比索，差一个数量级，画出来像崩盘其实一分没降`);
  }
  if (pending) {
    meta.push(`本产品另有 ${pending} 条观测待价格审计、未上图`
      + `（只画 audit_status=accepted；pending 是「还没审」，不是「审过了没问题」）`);
  }
  const droppedLines = r.dropped_series || [];
  if (droppedLines.length) {
    meta.push(`${droppedLines.length} 条渠道线真实观测不足 ${r.min_points || 2} 天未画：`
      + droppedLines.slice(0, 6).map(d => `${d.name}（${d.n_points} 点）`).join('、')
      + (droppedLines.length > 6 ? '…' : ''));
  }
 ...
 *    把篮子成分列出来，这条线代表什么就具体了。产品曲线没有篮子 → 清空。 */
function renderCurveBasket(r) {
