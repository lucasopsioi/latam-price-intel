...
/* ★ 品牌 + 型号的展示名（与 app/trends.py display_label、app/agents/weekly.py _label
 *   同一逻辑）。权威 SKU 的 model_name 自带品牌前缀（"Lenovo Tab"、"Apple iPad Air"），
 *   裸拼 brand + model_name 得到「Lenovo Lenovo Tab」。入库层不许剥品牌
 *   （跨品牌撞车的守卫靠它），所以去重只能在展示层做：
 *   型号里品牌词只留第一个；型号以品牌开头就不再前缀。后端给了 label 就优先用它。 */
function prodLabel(brand, model) {
  const b = String(brand ?? '').trim(), m0 = String(model ?? '').trim();
  if (!b) return m0;
  if (!m0) return b;
  const bl = b.toLowerCase();
  let seen = false;
  const out = [];
  for (const t of m0.split(/\s+/)) {
    if (t.toLowerCase() === bl) { if (seen) continue; seen = true; }
    out.push(t);
  }
  const m = out.join(' ') || m0;
  return m.toLowerCase().startsWith(bl) ? m : `${b} ${m}`;
}
/* 型号那一格：品牌已经单独显示（加粗 / 另一列）时，把型号打头的品牌词去掉。
 *   剥空则退回完整名 —— 「剥出空型号比留噪声危险」。 */
function modelPart(brand, model) {
  const full = prodLabel(brand, model);
  const b = String(brand ?? '').trim();
  if (!b) return full;
  const rest = full.toLowerCase().startsWith(b.toLowerCase())
    ? full.slice(b.length).trim() : full;
  return rest || full;
}
...
const PAGES = {
  ...
  voc: ['消费者口碑', loadVoc],
  intel: ['情报流', loadIntel],
  // 表格与只读概览一起加载；两个函数都在本文件里，直接引名会撞 TDZ，
  // 与 boards.js 那两个同理，统一包一层。
  products: ['我的产品', () => { loadGrid(); loadProducts(); }],
...
async function loadDash() {
  ...
  $('#dash-signals').innerHTML = sig.rows.length ? sig.rows.map(x => {
    ...
    return `<div style="padding:11px 13px;border-bottom:1px solid var(--line)">
      <div style="display:flex;gap:7px;align-items:center;flex-wrap:wrap">
        ...
        <b>${esc(x.brand || '')}</b>
        <span style="color:var(--text-2)">${esc(modelPart(x.brand, x.model_name) || '')}</span>
        ${x.confidence != null
  ...
  $('#dash-movers').innerHTML = `<thead><tr>
    ...
    <th class="num">现价</th><th class="num">变动</th><th>渠道</th>
      ...
      <td><b>${esc(r.brand || '—')}</b>
        <div class="trunc" style="font-size:11px;color:var(--text-2)">${esc(modelPart(r.brand, r.model_name) || '')}</div></td>
...
async function loadLaunches() {
  ...
  $('#launch-pending').innerHTML = d.pending_latam.length ?
    ...
    d.pending_latam.map(p => `<tr>
      <td>${esc(p.brand)}</td><td><b>${esc(modelPart(p.brand, p.model_name))}</b></td>
...
async function loadVoc() {
  ...
  $('#voc-profiles').innerHTML = d.profiles.length ? `<thead><tr>
    ...
    d.profiles.map(p => `<tr>
      <td>${esc(p.brand || '—')}</td>
      <td>${p.is_hot ? '🔥 ' : ''}<b>${esc(modelPart(p.brand, p.model_name) || '—')}</b></td>
  ...
  $('#voc-insights').innerHTML = d.insights.length ? d.insights.map(i => `
    <div class="card" style="margin-bottom:12px">
      <h3>${esc(prodLabel(i.brand, i.model_name))}
...
async function loadDecay() {
  ...
  if (s.length) {
    html += s.map(p => `<div style="margin-bottom:14px">
      <div style="font-size:12.5px;font-weight:600">${esc(prodLabel(p.brand, p.model))}
  ...
  if (ex.length) {
    html += `<div class="note" style="font-size:12px;margin-top:10px">
      ...
      <ul class="reason-list">${ex.slice(0, 8).map(e =>
        `<li>${esc(prodLabel(e.brand, e.model))} —— ${esc(e.reason)}</li>`).join('')}</ul>
...
const SECRET_META = {
  ...
  deepseek_api_key: ['DeepSeek API Key',
    '★★ 建议。出报告的写作模型（周报正文、小节标题、跨国分析）走 DeepSeek；不填则报告回落用 MiniMax 写。搜索/研判类 Agent 不受此项影响。',
    'https://platform.deepseek.com', 'nice'],
...
const PLAIN_META = {
  ...
  llm_model_deepseek: ['DeepSeek 模型名',
    '默认 deepseek-chat（官方别名，恒指最新）。要点名 V4 Pro 就填它的模型 id',
    '', 'optional'],
...
/** DeepSeek 模型下拉：选项来自**账号真实可用**的模型列表，不写死。
 *  取不到（没 Key / 接口挂了）就保留手填输入框 —— 列不出模型不能卡住配置。 */
async function loadModelPicker() {
  const slot = $('#ds-model-slot');
  if (!slot) return;
  let d;
  try { d = await api('/api/settings/models?provider=deepseek'); }
  catch (e) { return; }
  if (!d || !(d.models || []).length) {
    const why = d && d.reason ? ` <span class="hint">（${esc(d.reason)}，可手填）</span>` : '';
    slot.insertAdjacentHTML('beforeend', why);
    return;
  }
  const cur = d.current || '';
  slot.innerHTML =
    `<label class="field"><span>DeepSeek 模型 <span class="tag ok">${d.models.length} 个可用</span></span>
       <select data-key="llm_model_deepseek">` +
    d.models.map(m =>
      `<option value="${esc(m)}"${m === cur ? ' selected' : ''}>${esc(m)}</option>`).join('') +
    `</select>
     <span class="hint">出周报的写作模型。列表来自你 DeepSeek 账号的实际权限；
     换了模型点「保存」即可，下一期报告就用新模型。</span></label>`;
}
...
async function loadSettings() {
  ...
  $('#set-secrets').innerHTML =
    ...
    `<div id="ds-model-slot">` + field('llm_model_deepseek', PLAIN_META, 'text') + `</div>` +
    `<div class="row" style="margin-top:10px">
       <button onclick="testKeys()">🔌 测试连接</button>
       <span class="hint" id="key-test-result"></span>
     </div>`;
  // 有 Key 就把手填框换成真实可选的下拉（模型会上下线，写死的候选迟早对不上）
  loadModelPicker();
...
async function loadTrend() {
  ...
  $('#trend-list').innerHTML = `<thead><tr><th>品牌 / 型号</th><th class="num">观测天</th>
    ...
    (d.rows.length ? d.rows.map(r => `<tr class="clickable ${r.id === TREND_SEL ? 'sel' : ''}"
      ...
      <td><b>${esc(r.brand)}</b> ${esc(modelPart(r.brand, r.model_name))}
...
async function showTrend(pid) {
  ...
  $('#trend-title').innerHTML =
    `${esc(d.product.label || prodLabel(d.product.brand, d.product.model_name))} 价格走势`;

  // ★★ 后端已切到 audit_status='accepted'（pending 是"还没审"不是"审过了没问题"），
  //   而审计积压未清 —— 近 30 天整机观测 MX 100% / PE 95% / CO 89% 仍 pending。
  //   于是**大量有观测的产品此刻画不出线**。写死一句"没有价格"就是在替这些产品
  //   宣布"这款货没有价格"，而真相是"它的价格还在待审队列里"。
  //   两种状态在界面上长得一样，就只能靠文案分开说 —— 后端专门给了 pending_obs /
  //   unclassified_obs 就是干这个的，不读它等于把接口给的证据扔掉。
  const pend = Number(d.pending_obs || 0);
  const pendDays = Number(d.pending_days || 0);
  const unk = Number(d.unclassified_obs || 0);
  const unkDays = Number(d.unclassified_days || 0);
  // ★ note 里写着口径、被挡掉多少、哪些渠道线没画 —— 任何分支都要渲染。
  //   以前只在 `!d.trendable` 时渲染，而空图那条分支直接 return，note 一个字都到不了页面。
  const noteHtml = d.note ? `<div class="note warn">${esc(d.note)}</div>` : '';

  if (!d.series.length) {
    const blocked = [];
    if (pend) blocked.push(`${pend} 条（${pendDays} 个采集日）待价格审计`);
    if (unk) blocked.push(`${unk} 条（${unkDays} 个采集日）整机/配件未判定`);
    const body = blocked.length
      ? `<p>审计通过的观测还画不出曲线 —— 另有 ${esc(blocked.join('、'))}，未上图。</p>
         <p style="font-size:11px">这不是"没有价格"：只画审计通过（accepted）的观测，
         审计积压清完后这条曲线会自动补上。</p>`
      : '<p>这个产品在当前筛选下没有价格</p>';
    $('#trend-chart').innerHTML = `<div class="empty">${body}</div>` + noteHtml;
    return;
  }
  $('#trend-chart').innerHTML = d.series.map(s => trendSvg(s)).join('') + noteHtml;
}
...
/* ══════════════════ 我的产品：在线表格（2026-09-07）══════════════════
   原来只能「下载模板→填→导入」，改一个字要走一遍 Excel。
   ★ 分国定价那张表当时是**空的**（0 行）——竞品匹配取不到我方价，
     207 个「产品×国家」组合算不出来，根因就在这里没人填。 */
const GRID_DEFS = {
  product: {
    label: '产品', freezeCols: 1,
    columns: [
      { key: 'marketing_name', title: '营销名', width: 190 },
      { key: 'category_code', title: '产业', width: 82 },
      { key: 'internal_code', title: '内部代号', width: 110 },
      { key: 'cert_model', title: '认证型号', width: 110 },
      { key: 'series', title: '系列', width: 90 },
      { key: 'chipset', title: '芯片', width: 140 },
      { key: 'screen', title: '屏幕', width: 170 },
      { key: 'launch_earliest', title: '最早发货', width: 100 },
      { key: 'launch_latest', title: '最晚发货', width: 100 },
      { key: 'status', title: '状态', width: 80 },
      { key: 'notes', title: '备注', width: 160 },
    ],
  },
  sku: {
    label: 'SKU', freezeCols: 1,
    columns: [
      { key: 'product_name', title: '产品', width: 190, readonly: true },
      { key: 'product_id', title: '产品ID', width: 74, type: 'number' },
      { key: 'sku_name', title: 'SKU 名', width: 110 },
      { key: 'color', title: '颜色', width: 100 },
      { key: 'ram_gb', title: 'RAM(GB)', width: 86, type: 'number' },
      { key: 'rom_gb', title: 'ROM(GB)', width: 86, type: 'number' },
      { key: 'ean_code', title: 'EAN', width: 140 },
      { key: 'notes', title: '备注', width: 150 },
    ],
  },
  pricing: {
    label: '分国定价', freezeCols: 1,
    columns: [
      { key: 'product_name', title: '产品', width: 190, readonly: true },
      { key: 'product_id', title: '产品ID', width: 74, type: 'number' },
      { key: 'country_code', title: '国家', width: 70 },
      { key: 'currency', title: '币种', width: 70 },
      { key: 'rrp_local', title: 'RRP 本币', width: 110, type: 'number' },
      { key: 'street_price_local', title: '实际街价', width: 110, type: 'number' },
      { key: 'discount_pct', title: '常促折扣%', width: 96, type: 'number' },
      { key: 'fx_rate', title: '汇率', width: 86, type: 'number' },
      { key: 'on_sale', title: '在售(1/0)', width: 86, type: 'number' },
      { key: 'sales_volume_band', title: '销量档', width: 120 },
      { key: 'sales_period', title: '销量周期', width: 130 },
    ],
  },
};
let _grid = null, _gridKind = 'product';

async function loadGrid(kind) {
  _gridKind = kind || _gridKind;
  const def = GRID_DEFS[_gridKind];
  const host = $('#prod-grid');
  if (!host) return;
  $$('#prod-grid-tabs button').forEach(b =>
    b.classList.toggle('on', b.dataset.kind === _gridKind));
  let d;
  try { d = await api('/api/products/grid/' + _gridKind); }
  catch (e) { host.innerHTML = `<div class="empty"><p>读取失败：${esc(e.message)}</p></div>`; return; }
  $$('#prod-grid-tabs button').forEach(b => {
    if (b.dataset.kind === _gridKind) {
      const c = b.querySelector('.cnt'); if (c) c.textContent = d.rows.length;
    }
  });
  _grid = new Grid(host, {
    columns: def.columns, rows: d.rows, freezeCols: def.freezeCols,
    rowKey: r => (r.id == null ? null : r.id),
  });
  _grid.onSave = async changes => {
    if (!changes.length) return;
    try {
      const r = await api('/api/products/grid/' + _gridKind,
        { method: 'POST', body: JSON.stringify({ changes }) });
      let msg = `已保存：更新 ${r.updated}，新增 ${r.created}`;
      if (r.skipped) msg += `，跳过 ${r.skipped}`;
      if (r.revoked_matches) msg += `；品类变更作废了 ${r.revoked_matches} 条竞品匹配（下轮重算）`;
      toast(msg, 5000);
      if (r.errors && r.errors.length) alert('部分单元格未写入：\n\n' + r.errors.join('\n'));
      _grid.markSaved();
      loadGrid(_gridKind);
      loadProducts();
