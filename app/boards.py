...
def pending_blocked(base_where: list[str], params: list, join: str = "") -> dict:
    """被审计门（audit_status = 'accepted'）挡在图外的观测数。

    ★ 单一实现、四张图共用：口径必须与那张图**逐字同源**（同一份 base_where + params，
      只把 accepted 换成 pending），否则报出来的"待审计条数"和图上缺的那块对不上 ——
      对不上的数比不报更能骗人。
    ★ 为什么必须报：2026-09-04 四张图集体切到 = 'accepted'，而积压没清完
      （近 30 天 MX 100% / PE 95% / CO 90% pending）。图上留白与"这个国家没在打价格战"
      在界面上长得一模一样，而后者是错的（silent-failures-detection：没审 ≠ 没有）。
    返回 {"pending_obs": n, "pending_days": d, "note": "…"}；n=0 时 note 为空串。
    """
    r = db.q1(f"""
        SELECT COUNT(*) AS n, COUNT(DISTINCT po.obs_date) AS days
        FROM price_obs po {join}
        WHERE {' AND '.join(base_where)} AND po.audit_status = 'pending'
    """, params) or {}
    n, d = int(r.get("n") or 0), int(r.get("days") or 0)
    return {"pending_obs": n, "pending_days": d, "audit_filter": "accepted",
            "note": (f"另有 {n:,} 条观测（{d} 个采集日）仍待价格审计（pending），未进这张图 —— "
                     f"留白可能是「还没审」而不是「没有动作」；审计积压清完后会自动补上。"
                     if n else "")}
...
def _promo_base_where(country: str, since: str) -> tuple[list, list]:
    """促销收缩的基础过滤（不含审计门）—— 图与"被挡掉多少条"共用同一份，
    两条空态出口（历史不够 / 篮子不够）也共用它。抄成两份必然漂。"""
    where = ["po.obs_date >= ?", "po.product_kind = 'device'", "po.brand_id IS NOT NULL",
...
def promo_shrink(days: int = 14, country: str = "", min_basket: int = 12) -> dict:
    ...
    if span < 4:
        # ★ 连"历史不够"这条空态也要带上待审计计数：数据不够与数据没审是两个不同的结论，
        #   而它们在同一张空图上长得一样（口径复用 _promo_base_where，不另抄一份 where）。
        bw, bp = _promo_base_where(country, _back(int(days)))
        short = {"items": [], "insufficient": True, "window_days": 0,
                 "note": f"只有 {span + 1} 天连续观测，前后半窗各不足 2 天，环比无意义。"
                         f"促销收缩至少需要 4 天历史。"}
        b0 = pending_blocked(bw, bp)
        short.update({k: b0[k] for k in ("pending_obs", "pending_days", "audit_filter")})
        if b0["note"]:
            short["note"] += "　" + b0["note"]
        return short
    ...
    # ★ audit_status = 'accepted'（2026-09-04 切换，喂「促销收缩」哑铃图）。
    #   此前 <> 'rejected' 把 pending 当通过 —— 全库 84.5% 是 pending（审计每天只审 2000 行
    #   且按国家字母序分配名额，MX 永远 0），等于没审。
    #   ★ 前提：积压已清（tools/audit_backlog.py --apply 跑到 pending<阈值）再重启服务，
    #     否则 MX 等国 accepted=0，这张图整体留白。runbook 顺序写死，不要反。
    base_where, params = _promo_base_where(country, since)
    where = base_where + ["po.audit_status = 'accepted'"]
    blocked = pending_blocked(base_where, params)

    def _with_pending(d: dict) -> dict:
        """把"被审计门挡掉多少条"贴到每一种返回形态上 —— 尤其是**空态**那几种：
        这张图最常见的输出就是 insufficient/no_movement，而"篮子凑不齐"与
        "篮子里的行还没审"是两个完全不同的结论。"""
        d.update({k: blocked[k] for k in ("pending_obs", "pending_days", "audit_filter")})
        if blocked["note"]:
            d["note"] = (d.get("note") or "") + "　" + blocked["note"]
        return d
    ...
    if not out:
        return _with_pending({
                "items": [], "insufficient": True, "window_days": days,
    ...
    if not shrinkers:
        ...
        return _with_pending({
                "items": [], "window_days": days, "insufficient": True,
                "no_movement": True,
    ...
    if biggest < 1.0:
        ...
        return _with_pending({
                "items": out, "window_days": days, "insufficient": True,
                "no_movement": True,
                "window": {"from": since, "mid": mid, "to": now}, "thin": thin[:8],
    ...
    return _with_pending({
            "items": out, "window_days": days, "insufficient": False,
            "no_movement": False,
            "window": {"from": since, "mid": mid, "to": now}, "thin": thin[:8],
...
def price_band(country: str = "", category: str = "", days: int = 7,
    ...
    #   宁可少报几个品牌，不可报一个错的价格段。
    # ★ audit_status = 'accepted'（2026-09-04 切换，喂「价格带卡位」P25/中位/P75）。
    #   只用审计通过的行：品类地板（price_audit.FLOOR_X）剔掉的 8,500 CLP 钢化膜这类
    #   "device" 行，此前以 pending 身份直接进了价格带。前提同上：积压已清再切服务。
    base_where = ["po.obs_date >= ?", "po.country_code = ?", "po.sale_price IS NOT NULL",
                  "po.product_kind = 'device'",
                  "po.condition = 'new'", "po.is_bundle = 0", "po.brand_id IS NOT NULL"]
    params: list = [_back(days), country.upper()]
    if category:
        base_where.append("po.category_code = ?")
        params.append(category)
    where = base_where + ["po.audit_status = 'accepted'"]
    blocked = pending_blocked(base_where, params)
    ...
    kept = out[:limit]
    # ★ 空图/缺品牌时必须说得清是"没审"还是"没有"：这张图锁单国单品类，
    #   MX 近 30 天 100% pending 时它会整张留白，而留白本身没有任何说明。
    return {"items": kept, "country": country.upper(), "category": category,
            ...
            "pending_obs": blocked["pending_obs"], "pending_days": blocked["pending_days"],
            "audit_filter": blocked["audit_filter"],
            "note": blocked["note"] or (
                "" if kept else "这个国家/品类在窗口内没有已审计通过的整机观测。")}
...
def discount_heat(days: int = 7, min_n: int = 10) -> dict:
    ...
    """
    # ★ audit_status = 'accepted'（2026-09-04 切换，喂「折扣力度」国家×品类热力图与其中性点）。
    #   格子与中性点两处必须同口径切，否则中性点是全量算的、格子是已审算的，色阶分界会漂。
    #   前提：积压已清再切服务（否则 MX 一列全是 None，看着像"没折扣数据"）。
    rows = db.q("""
        SELECT country_code cc, category_code cat, COUNT(*) n, AVG(discount_pct) d
        FROM price_obs
        WHERE obs_date >= ? AND audit_status = 'accepted'
    ...
    base = db.q1("""SELECT AVG(discount_pct) d FROM price_obs
                    WHERE obs_date >= ? AND discount_pct > 0
                      AND audit_status = 'accepted'""", (_back(days),)) or {}
    # ★ 被审计门挡掉的格子：口径与上面的格子查询逐字同源（只把 accepted 换成 pending）。
    #   这张图的空格已经有一层含义（样本不足 → 画"·"），再叠一层"没审"必须能分开 ——
    #   否则 MX 一整列 None 看着像"墨西哥不打折"。
    pend_rows = db.q("""
        SELECT country_code cc, category_code cat, COUNT(*) n
        FROM price_obs
        WHERE obs_date >= ? AND audit_status = 'pending'
          AND product_kind <> 'accessory' AND discount_pct > 0
          AND country_code IS NOT NULL AND category_code IS NOT NULL
        GROUP BY 1, 2
    """, (_back(days),))
    pend = {(r["cc"], r["cat"]): r["n"] for r in pend_rows}
    ...
    for cat in cats:
        for cc in ccs:
            ...
            cells.append({"x": cc, "y": CAT_ZH.get(cat, cat), "cat": cat,
                          "v": v, "n": (r or {}).get("n", 0),
                          "pending": p,
                          # 空格的两种原因要分得开：样本不足 vs 还没审
                          "audit_blocked": bool(v is None and p)})
    pending_obs = sum(pend.values())
    blocked_cells = sum(1 for c in cells if c["audit_blocked"])
    return {"xs": ccs, "ys": [CAT_ZH.get(c, c) for c in cats], "cells": cells,
            "center": round(base.get("d") or 0, 1), "days": days, "min_n": min_n,
            "pending_obs": pending_obs, "blocked_cells": blocked_cells,
            "audit_filter": "accepted",
            "note": (f"另有 {pending_obs:,} 条在促观测仍待价格审计（pending），未进热力图；"
                     f"其中 {blocked_cells} 个空格是**因为还没审**，不是没折扣。"
                     if pending_obs else "")}
...
def seller_spread(country: str = "", category: str = "", days: int = 7,
    ...
    """
    # ★ audit_status = 'accepted'（2026-09-04 切换，喂「自营 vs 第三方价差」哑铃图）。
    #   第三方溢价正是审计要剔的东西；用 pending 行画这张图 = 把待审的溢价当事实。
    #   前提：积压已清再切服务。
    base_where = ["po.obs_date >= ?", "po.sale_price IS NOT NULL",
                  "po.rival_product_id IS NOT NULL", "po.product_kind = 'device'",
    ...
    # ★ 这张图要求同一产品**两类卖家都有** accepted 行；审计门下任一侧还没审就整条消失，
    #   而"渠道管控没问题"与"这一侧还没审"在哑铃图上都表现为"这条线不在"。
    return {"items": out[:limit], "total": len(out), "currency_mixed":
            len({r["currency"] for r in rows}) > 1,
            "pending_obs": blocked["pending_obs"], "pending_days": blocked["pending_days"],
            "audit_filter": blocked["audit_filter"],
            "note": blocked["note"] or (
                "" if out else "窗口内没有任何产品同时有自营与第三方的已审计报价。")}
