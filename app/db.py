...
MIGRATIONS: list[tuple[str, str, str]] = [
    ...
    # 价格口径（2026-08-28 用户拍板「公开价优先」）：
    # NULL/'public' = 任何人可买的公开价；'card' = 持卡人专享价
    # （智利/秘鲁的 TARJETA FEST、CMR、Cencosud 卡价）。
    # 卡价保留入库但不进任何价格分析 —— 竞品定价看的是公开市场价。
    ("price_obs", "price_scope", "TEXT"),
    # 品类判定的来源：NULL/'search-intent'（采集意图盖章，可能错）/ 'rule'
    # （extract.crosscheck_category）/ 'llm:<模型>'（LLM 兜底判定）。
    # 没有这一列就分不清哪些是"已判过的"，LLM 层每轮会重复判同一批产品。
    ("rival_product", "category_source", "TEXT"),
    # RAM/ROM 的来源：NULL = 从商品标题解析（第一手）；
    # 'product-spec' = 标题没写，但该观测的 ROM 与产品权威规格的 ROM 一致
    # （同变体），据此把 RAM 带过来。变体不一致或未知的**一律不推断** ——
    # 同一机型不同变体 RAM 不同，猜一个就是编造规格。
    ("price_obs", "spec_source", "TEXT"),
    # ★ 桶标记（2026-09-04）。is_bucket=1 表示这一行不是一款产品而是一个"系列/兜底名"
    #   （权威表的系列级 SKU「Lenovo Tab」、nubimetrics 前 4 词兜底名、白牌桶），
    #   里面装着多款不同产品 —— 它的价格曲线在 18 万与 8,500 CLP 之间来回跳，
    #   涨价榜会跑出「白牌 +1387%」。单品消费方（趋势入口、竞品候选池、变动检测）
    #   一律 `COALESCE(rp.is_bucket,0)=0`；可看但要标「系列·未细分」的地方带上它。
    #   bucket_reason 记判据（sku_rules:generic / nubimetrics:fallback / whitelabel），
    #   判据的单一实现在 app/skunorm.py bucket_verdict。
    ("rival_product", "is_bucket", "INTEGER NOT NULL DEFAULT 0"),
    ("rival_product", "bucket_reason", "TEXT"),
]
