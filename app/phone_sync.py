...
def _run_ps(script: str, workdir: str, timeout: int = 900) -> str:
    ...
    # ★★ 走 .ps1 文件而不是 -EncodedCommand（2026-09-07 修）：
    #   EncodedCommand 是命令行参数，受 Windows 32,767 字符上限约束。脚本长度随
    #   待同步文件数线性增长 —— 实测 20 个文件就是 39,289 字符脚本 / 104,770 字符
    #   编码后，**是上限的 3.2 倍**，必然 `FileNotFoundError: [WinError 206]
    #   文件名或扩展名太长`。日志里这条错误出现了 **2,232 次**：手机自动同步
    #   一直是坏的，而 APScheduler 把异常吞成一行 ERROR 后照样打 "executed
    #   successfully"，所以「同步失败」和「没有新文件」在界面上长得一模一样。
    #   ★ 中文脚本必须 UTF-8 **带 BOM**（本机 ANSI=cp936，无 BOM 时 PowerShell
    #     按 cp936 解码，中文日志变乱码 —— 知识页 windows-cp936-encoding）。
    ps1 = os.path.join(workdir, "sync.ps1")
