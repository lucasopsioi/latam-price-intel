...
def import_product_list(path: str | Path | None = None) -> dict:
    ...
    with open(path, "r", encoding="utf-8-sig", newline="") as f:
        for row in _csv.DictReader(f):
            ...
            with db.tx() as conn:
                if existing:
                    # ★ 品类是竞品匹配的输入之一：改了就必须让旧匹配失效，
                    #   否则会留下"我方耳机 vs 友商手表"这种跨品类脏行。
                    #   2026-09-03 用户截图撞见的正是它的**友商侧镜像**
                    #   （见 categorizer._apply 的同款处理）——那次是友商改品类，
                    #   这里是我方改品类，同一个病根的两边。
                    #   守卫与 matcher._persist 的 DELETE 逐字一致：
                    #   只作废自动算的，人工确认/排除的判断保留。
                    old = db.q1("SELECT category_code c FROM my_product WHERE id=?",
                                (existing["id"],))
                    conn.execute("""UPDATE my_product SET category_code=?, series=?,
                                    updated_at=datetime('now') WHERE id=?""",
                                 (cat, (row.get("product_family") or "").strip(),
                                  existing["id"]))
                    if old and (old["c"] or "") != cat:
                        n = conn.execute(
                            "DELETE FROM competitor_match WHERE my_product_id=?"
                            " AND source='auto' AND is_confirmed=0 AND is_excluded=0",
                            (existing["id"],)).rowcount or 0
                        if n:
                            report.setdefault("matches_invalidated", 0)
                            report["matches_invalidated"] += n
                            log.info("我方产品 #%s 品类 %s→%s，作废 %d 条自动匹配"
                                     "（下轮竞品匹配会重算）",
                                     existing["id"], old["c"], cat, n)
                    report["updated"] += 1
                else:
                    conn.execute("""
...
# ══════════════════ 在线表格的读写（2026-09-07 用户要 Excel 式录入）══════════════════
#
# ★ 白名单是硬闸：可写的列写死在这里，前端传什么都不能越界。
#   直接把前端字段名拼进 SQL 等于把表结构交给浏览器 —— 而这三张表是
#   竞品匹配的输入，写坏一列，下游全错且不报错。
GRID_TABLES = {
    "product": {
        "table": "my_product",
        "cols": ["marketing_name", "internal_code", "cert_model", "category_code",
                 "series", "chipset", "screen", "launch_earliest", "launch_latest",
                 "status", "notes"],
        "num": set(),
    },
    "sku": {
        "table": "my_sku",
        "cols": ["product_id", "sku_name", "color", "ram_gb", "rom_gb",
                 "ean_code", "notes"],
        "num": {"product_id", "ram_gb", "rom_gb"},
    },
    "pricing": {
        "table": "my_pricing",
        "cols": ["product_id", "sku_id", "country_code", "rrp_local", "currency",
                 "fx_rate", "street_price_local", "discount_pct", "on_sale",
                 "sales_volume_band", "sales_period"],
        "num": {"product_id", "sku_id", "rrp_local", "fx_rate",
                "street_price_local", "discount_pct", "on_sale"},
    },
}


def grid_rows(kind: str) -> dict:
    """给在线表格取数。分国定价把产品名带出来，否则一屏全是 id 没法录。"""
    spec = GRID_TABLES.get(kind)
    if not spec:
        raise ValueError(f"未知表：{kind}")
    if kind == "product":
        rows = db.q("""SELECT mp.id, mp.marketing_name, mp.internal_code, mp.cert_model,
                              mp.category_code, mp.series, mp.chipset, mp.screen,
                              mp.launch_earliest, mp.launch_latest, mp.status, mp.notes
                       FROM my_product mp LEFT JOIN category c ON c.code=mp.category_code
                       ORDER BY c.sort_order, mp.marketing_name""")
    elif kind == "sku":
        rows = db.q("""SELECT s.id, s.product_id, mp.marketing_name AS product_name,
                              s.sku_name, s.color, s.ram_gb, s.rom_gb, s.ean_code, s.notes
                       FROM my_sku s LEFT JOIN my_product mp ON mp.id=s.product_id
                       ORDER BY mp.marketing_name, s.ram_gb, s.rom_gb""")
    else:
        rows = db.q("""SELECT p.id, p.product_id, mp.marketing_name AS product_name,
                              p.sku_id, p.country_code, p.rrp_local, p.currency,
                              p.fx_rate, p.street_price_local, p.discount_pct,
                              p.on_sale, p.sales_volume_band, p.sales_period
                       FROM my_pricing p LEFT JOIN my_product mp ON mp.id=p.product_id
                       ORDER BY mp.marketing_name, p.country_code""")
    return {"rows": rows, "editable": spec["cols"]}


def grid_save(kind: str, changes: list[dict]) -> dict:
    """把表格改动落库。changes: [{key: 行主键(新行为 null), fields: {列: 值}}]

    ★ 品类改了要作废竞品匹配 —— 与 import_product_list 同一条纪律：
      品类是匹配的输入，改了不作废就会留下「我方耳机 vs 友商手表」那种脏行。
    """
    spec = GRID_TABLES.get(kind)
    if not spec:
        raise ValueError(f"未知表：{kind}")
    tbl, allow, nums = spec["table"], set(spec["cols"]), spec["num"]
    n_upd = n_new = n_bad = 0
    revoked = 0
    errors: list[str] = []
    with db.tx() as conn:
        for ch in changes or []:
            fields = {k: v for k, v in (ch.get("fields") or {}).items() if k in allow}
            if not fields:
                n_bad += 1
                continue
            for k in list(fields):
                ...
                if v == "" or v is None:
                    ...
                    try:
                        fields[k] = float(v) if "." in str(v) else int(v)
                    except (TypeError, ValueError):
                        errors.append(f"{k} 需要数字，收到 {v!r}，该列跳过")
                        del fields[k]
            if not fields:
                n_bad += 1
                continue
            ...
            if rid in (None, "", "new"):
                cols = ",".join(fields)
                qs = ",".join("?" * len(fields))
                conn.execute(f"INSERT INTO {tbl}({cols}) VALUES({qs})",
                             tuple(fields.values()))
                n_new += 1
                continue
            # 改品类前先取旧值，用来判断要不要作废匹配
            old_cat = None
            if kind == "product" and "category_code" in fields:
                r = conn.execute("SELECT category_code FROM my_product WHERE id=?",
                                 (rid,)).fetchone()
                old_cat = r[0] if r else None
            sets = ",".join(f"{k}=?" for k in fields)
            extra = ", updated_at=datetime('now')" if kind == "product" else ""
            cur = conn.execute(f"UPDATE {tbl} SET {sets}{extra} WHERE id=?",
                               (*fields.values(), rid))
            n_upd += cur.rowcount or 0
            if old_cat is not None and fields.get("category_code") != old_cat:
                revoked += conn.execute(
                    "DELETE FROM competitor_match WHERE my_product_id=? AND source='auto'"
                    " AND is_confirmed=0 AND is_excluded=0", (rid,)).rowcount or 0
    if revoked:
        log.info("在线表格改了品类，作废 %d 条自动匹配（下轮重算）", revoked)
    return {"updated": n_upd, "created": n_new, "skipped": n_bad,
            "revoked_matches": revoked, "errors": errors[:8]}
