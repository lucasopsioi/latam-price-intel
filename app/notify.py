...
def build_daily_brief(for_date: str | None = None) -> str:
    ...
    # —— 价格异动 ——
    # ★ audit_status = 'accepted'（2026-09-04 切换，喂 Telegram 日报的「价格异动 ≥5%」段）。
    #   此前 <> 'rejected' 把 pending 当通过：8,500 CLP 钢化膜（判 device、挂到 Lenovo Tab）
    #   就是这样进过"异动"。今天价与上一次价**两边都**只认 accepted，否则是在拿已审 vs 未审比。
    #   ★ 前提：积压已清（tools/audit_backlog.py --apply）且流水线按观测日审到 pending=0。
    #     日报 08:30 跑时当天采集才开 1 小时、行全是 pending —— 所以这里播的是
    #     **最近一个审完的观测日**（该日 pending=0 且有 accepted），并把"今天还没审"说出来：
    #     "没审"和"没异动"不能长得一样（silent-failures-detection）。
    audited = db.q1("""
        SELECT obs_date d FROM price_obs WHERE obs_date <= ?
        GROUP BY obs_date
        HAVING SUM(audit_status='pending') = 0 AND SUM(audit_status='accepted') > 0
        ORDER BY obs_date DESC LIMIT 1
    """, (d,)) or {}
    move_date = audited.get("d") or d
    unaudited = db.q1("""SELECT COUNT(*) n FROM price_obs
                         WHERE obs_date = ? AND audit_status = 'pending'""", (d,)) or {}
    moves = db.q("""
    ...
    """, (move_date,))
    if moves:
        tag = f"，观测日 {move_date}" if move_date != d else ""
        lines.append(f"<b>💰 价格异动 ≥5%（{len(moves)}{tag}）</b>")
        for m in moves:
        ...
        lines.append("")
    if (unaudited.get("n") or 0) and move_date != d:
        lines.append(f"⏳ {d} 有 {unaudited['n']:,} 条价格观测尚未审计，"
                     f"价格异动按最近审完的观测日 {move_date} 播报")
        lines.append("")
