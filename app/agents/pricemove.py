        ...
        rows = db.q("""
            WITH cur AS (
              ...
              WHERE po.obs_date = ?
                ...
                -- 持卡人专享价不进比价（2026-08-28 用户拍板「公开价优先」）
            ...
            -- ★ 同日多价取**自营价优先**（2026-08-28 用户拍板）：
            --   自营/品牌官方的价签是厂商定价动作；没有自营才退到
            --   第三方最低价 —— 一个乱标价的第三方卖家不该触发假变动。
            , cur_agg AS (
              SELECT country_code, channel_id, brand_id, category_code,
                     rival_product_id, sku_key, currency, channel_kind,
                     COALESCE(MIN(CASE WHEN seller_kind IN
              ...
              SELECT po.rival_product_id, po.channel_id, po.country_code, po.currency,
                     CASE WHEN po.url IS NOT NULL AND po.url <> ''
                          ...
                          ELSE 's:' || IFNULL(po.rom_gb,-1) || '|'
                               ...
                               || IFNULL(po.color,'') END AS sku_key,
                     po.obs_date,
                     COALESCE(MIN(CASE WHEN po.seller_kind IN
                                ('self_operated','brand_official')
