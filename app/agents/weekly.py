...
def _label(brand: str | None, model: str | None) -> str:
    ...
    if not m:
        return b
    # ★ 品牌词在型号里可能出现多次（实测 66 个产品形如 "Honor Honor 70 Honor"，
    #   来自标题里品牌被重复书写）——只留第一个，其余去掉。
...
def _cfg_tag(row: dict) -> str:
    """机型后面的配置尾巴：256GB / 8+256GB。

    ★ 用户 2026-08-28：「什么机型什么配置都要了解清楚」——
      同一机型不同存储是不同价位段，只写机型名会让读者以为在比同一件东西。
      解析不出配置就留空，不猜。
    """
    rom, ram = row.get("rom_gb"), row.get("ram_gb")
    if not rom:
        return ""
    rom_s = f"{rom // 1024}TB" if rom >= 1024 and rom % 1024 == 0 else f"{rom}GB"
    return f" {ram}+{rom_s}" if ram else f" {rom_s}"
...
class WeeklyReportAgent(BaseAgent):
    ...
    def run(self, week_start: str | None = None, scope: str = "all",
        ...
        # ★ as-of 基准：所有"近 N 天"窗口一律相对**报告期末**，不是相对今天。
        #   否则出历史周报（用户 2026-08-28：「不管今天的数据，先看历史数据
        #   输出周报，比如上一周的」）会把周期之后发生的事混进来 ——
    ...
    def _asof_date(self) -> str:
        """所有回看窗口的基准日（报告期末）。未设时退回今天。"""
        return getattr(self, "_asof", None) or db.today()
    ...
    def _coverage30(self) -> dict:
        ...
        rows = _safe_q("""
            SELECT country_code cc, category_code cat,
                   ...
                   MAX(obs_date) last_d
            FROM price_obs
            WHERE obs_date BETWEEN date(?,'-30 day') AND ?
              AND category_code IS NOT NULL
            GROUP BY cc, cat""", (self._asof_date, self._asof_date))
    ...
    def _moves30(self, cat: str) -> list[dict]:
        ...
        rows = _safe_q(f"""
            SELECT pm.country_code cc, COALESCE(pm.category_code,'unknown') cat,
                   ...
                   (SELECT po.rom_gb FROM price_obs po
                     WHERE po.rival_product_id=pm.rival_product_id
                       AND po.channel_id=pm.channel_id AND po.rom_gb IS NOT NULL
                     ORDER BY po.obs_date DESC LIMIT 1) rom_gb,
                   (SELECT po.ram_gb FROM price_obs po
                     WHERE po.rival_product_id=pm.rival_product_id
                       AND po.channel_id=pm.channel_id AND po.ram_gb IS NOT NULL
                     ORDER BY po.obs_date DESC LIMIT 1) ram_gb,
            ...
            LEFT JOIN rival_product rp ON rp.id=pm.rival_product_id
            LEFT JOIN channel ch ON ch.id=pm.channel_id
            WHERE pm.move_date BETWEEN date(?,'-30 day') AND ?
              AND ABS(pm.change_pct) >= ? {cat_f}
            ORDER BY ABS(pm.change_pct) DESC
        """, ([self._asof_date, self._asof_date, MOVE_ALERT_PCT]
              + ([cat] if cat else [])))
        out, dropped_old = [], 0
        for r in rows:
    ...
    _CLEAN_SQL = f"""
            ...
            WHERE po.country_code=? AND po.category_code=?
              ...
              AND po.is_bundle=0 AND po.audit_status<>'rejected'
              AND COALESCE(po.price_scope,'public') <> 'card'{_ACC_SQL}
    """
    ...
    def _mover_series(self, mover: dict, days_back: int = 35):
        ...
        # 自营价优先（2026-08-28 用户拍板）；卡专享价不进曲线
        rows = _safe_q("""
            SELECT po.obs_date d,
                   COALESCE(AVG(CASE WHEN po.seller_kind IN
                              ('self_operated','brand_official')
                            THEN po.sale_price END),
                            AVG(po.sale_price)) p
            FROM price_obs po
            WHERE po.rival_product_id=? AND po.channel_id=?
              AND po.sale_price>0 AND po.audit_status<>'rejected'
              AND COALESCE(po.price_scope,'public') <> 'card'
              AND po.obs_date BETWEEN date(?, ?) AND ?
            GROUP BY po.obs_date ORDER BY po.obs_date
        """, (pid, chid, self._asof_date, f"-{days_back} day",
              self._asof_date))
        if len(rows) < 2:
            return None, None, None, ""
        import datetime as _dt
        d0 = _dt.date.fromisoformat(rows[0]["d"])
        # ★ 补点补到**报告期末**，不是今天 —— 否则历史周报的曲线
        #   会一路延伸到今天（实测 8/17~8/23 的报告 x 轴画到 8/28）
        dn = _dt.date.fromisoformat(self._asof_date)
    ...
    def _top_products(self, cc: str, cat: str, currency: str,
        ...
        rows = _safe_q(f"""
            ...
            {self._CLEAN_SQL}
              AND po.obs_date BETWEEN date(?,'-35 day') AND ?
              AND po.rival_product_id IS NOT NULL
            GROUP BY po.rival_product_id, po.channel_id
            HAVING dz >= 5
        """, (cc, cat, currency, self._asof_date, self._asof_date))
        out = []
        for r in rows:
            ...
            for cc in ccs:
                ...
                if X["uniq"]:
                    out.append(_table(
                        ...
                        [[_label(m["brand"], m["model"]) + _cfg_tag(m),
    ...
    def _brief_focus(self, ws: str, we: str) -> str:
        ...
        for a in _safe_q("""SELECT pa.watch_id, pa.alert_date, pa.change_pct,
                            ...
                            LEFT JOIN rival_product rp ON rp.id=pa.rival_product_id
                            LEFT JOIN channel c ON c.id=pa.channel_id
                            WHERE pa.alert_date BETWEEN date(?,'-30 day') AND ?
