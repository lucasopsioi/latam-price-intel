...
from .categorizer import CategoryAgent
...
class Orchestrator:
    ...
    def rotation_category(self, d: date | None = None) -> str | None:
        # ★ 2026-08-28 用户拍板：**每日全品类**，轮换制废除。
        #   轮换是单品类每周只有 2-3 个观测日的采集端根因 ——
        #   曲线断点、周线点少、变动发现晚 2-3 天全由它来。
        #   返回 None = 不按轮值限制（计划覆盖全部启用品类）。
        #   代价（已向用户交底）：单轮时长约 2.5 倍、风控暴露升高。
        #   回退开关：runtime.yaml scrape.daily_all_categories: 0
        if (self.cfg.get("scrape") or {}).get("daily_all_categories", 1):
            return None
        d = d or date.today()
    ...
    def run_daily(self) -> dict:
        ...
        # ★★ 后处理必须按**本批次真实写入的观测日期**来跑，不能用 today()。
        #   2026-09-03 实测事故：采集 12:30 开跑、跨午夜到次日 07:51 才结束，
        #   而清洗/变动检测默认 obs_date=db.today() —— 此时已是第二天，
        #   去处理"第二天的数据"，而观测全部标着开跑那天的日期 ⇒ 一条都匹配不上。
        #   后果：8/14、8/15、8/16、8/19、8/22、8/29、8/30、9/01、9/02、9/03
        #   十天的观测**完全没挂上产品**（挂接率 0~2%，正常日是 95%），
        #   报告的产品维度分析等于丢了这些天。
        #   同类批次当天跑完的（8/24~8/28）则完全正常 —— 这正是它难被发现的原因。
        obs_dates = [r["d"] for r in db.q(
            "SELECT DISTINCT obs_date d FROM price_obs WHERE run_id=? ORDER BY d",
            (run_id,))] or [db.today()]
        log.info("本批次观测日期：%s", "、".join(obs_dates))

        # ③ 清洗（逐个观测日跑，跨午夜的批次才不会漏）
        clean_stats = self._stage("清洗", lambda: self._for_dates(
            obs_dates, lambda d: CleanerAgent(
                self.llm, self.cfg["agents"], plan_id=plan.get("plan_id")).run(obs_date=d)))

        # ③b 品类判定：规则层弃权的交 LLM 兜底。必须在审计**之前** ——
        #     审计按品类判合理价区间，品类错了审计跟着错。
        #     rematch=False：品类改判会让竞品匹配失效，CategoryAgent 单跑时
        #     会自己重算；但这里 ⑦ 紧接着就 rebuild_all，再算一遍纯浪费。
        #     失效动作（作废受影响的 auto 匹配）与 rematch 无关，照常执行。
        cat_stats = self._stage("品类判定", lambda: CategoryAgent(
            self.llm, self.cfg["agents"],
            plan_id=plan.get("plan_id")).run(rematch=False))

        # ④ 审计 —— ★ 2026-09-04 改：按本批次的真实观测日期循环、每日审到取尽。
        #   旧写法 `.run()` = 审"今天"、LIMIT 2000、ORDER BY country_code：
        #   跨午夜批次审的是次日那几百行；2000 个名额按字母序永远先给 BR/CL，
        #   MX 近 30 天 28,691 条整机观测 accepted=0、PE 94% pending —— 而所有看板
        #   用 `<> 'rejected'` 取数，等于整套分析跑在未审数据上。
        #   run_all(obs_date=d) 按游标循环到该日 pending 取尽（规则层零 token，
        #   灰区才进模型），与 Cleaner / PriceMove 同一条"按 obs_dates 逐日"的纪律。
        if self.cfg["agents"].get("enable_price_audit", True):
            def _audit_day(d: str) -> dict:
                r = PriceAuditAgent(self.llm, self.cfg["agents"],
                                    plan_id=plan.get("plan_id")).run_all(obs_date=d) or {}
                if r.get("warning"):
                    self.warnings.append(f"[{d}] {r['warning']}")
                return r
            audit_stats = self._stage("价格审计", lambda: self._for_dates(obs_dates, _audit_day))
            if audit_stats.get("pending_left"):
                self.warnings.append(
                    f"价格审计未取尽：仍有 {audit_stats['pending_left']} 条 pending"
                    f"（轮数上限或中途停止），这些行在 = 'accepted' 口径下不会进图")
        ...
        #     在脏数据上比价会播报出根本不存在的降价。
        move_stats = self._stage("价格变动检测", lambda: self._for_dates(
            obs_dates, lambda d: PriceMoveAgent(
                self.llm, self.cfg["agents"], plan_id=plan.get("plan_id")).run(obs_date=d)))
        ...
        # 跨品类不变量自检的告警要冒到本轮总结里 —— 只写日志没人看
        if match_stats.get("warning"):
            self.warnings.append(match_stats["warning"])
    ...
    @staticmethod
    def _for_dates(dates: list, fn):
        """对本批次涉及的每个观测日各跑一次，汇总计数。

        跨午夜的采集会写入两个 obs_date，只跑一个就会漏掉另一个。
        """
        total: dict = {}
        for d in dates:
            r = fn(d) or {}
