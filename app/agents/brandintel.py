...
from .llm import K_RULE, K_SPEC, as_dict, as_dicts, batch_key, pick_by_key
...
class BrandIntelAgent(BaseAgent):
    ...
    def _classify(self, rows: list[dict], brands: list[dict]) -> list[dict]:
        ...
        for i in range(0, len(rows), BATCH):
            ...
            keys = [batch_key(r["id"]) for r in chunk]     # 核对码：定位只认它，不信模型序号
            lines = []
            for j, r in enumerate(chunk):
                extra = (r.get("raw_text") or "")[:200].replace("\n", " ")
                lines.append(f"{j}. (k={keys[j]}) [{r.get('source_name') or '?'}|"
            ...
            prompt = (
                ...
                + "\n".join(lines) + "\n\n"
                "只返回 JSON 数组，每条：\n"
                '{"idx":序号,' + K_SPEC + ',"action":"上面八类之一或none","brand":"品牌名或null",'
            ...
            # ★ 按核对码对回原新闻，不信模型写的序号（llm.pick_by_key）。
            #   抄错的整条不采信 —— 品牌闸拦不住"品牌对、内容错"（原文恰好提到那个品牌）。
            picked, kstats = pick_by_key(parsed, keys)
            if kstats["rejected"]:
                self.log_step("动作归类序号核对", input_ref=f"batch:{i}", parsed=kstats,
                              decision="rejected", status="degraded",
                              reason="核对码抄错 = 内容属于别的新闻，整条不采信")
            for j, item in picked:
                rec = self._accept(item, chunk[j], alias_map, name2id, valid_cc)
                if rec is None:
                    continue
    ...
    def _accept(self, item: dict, src: dict, alias_map: dict,
                name2id: dict, valid_cc: set):
        """把模型返回的一条校验成可用记录。任何一关不过就丢。

        src 是按核对码对回来的原新闻（llm.pick_by_key），不再信模型写的序号。
        ...
        cand = resolve_brand(item.get("brand"), name2id, alias_map)   # 英文/别名/中文名
...
# ★ 模型报的品牌名有时是中文（2026-09-04 实测 run 339：苹果×9、小米×3、Acme、三星），
#   而 brand.name / aliases 全是英文（Acme的「Acme」别名还被 len>=3 剔掉）——
#   「苹果」解析不到 ⇒ 品牌核对失败 ⇒ 一整批 Apple 新闻全部无主。
#   prompt 已要求写英文，但模型行为不能靠 prompt 保证，代码侧必须能解析。
#   只收追踪品牌的常见中文写法；映射到英文名后仍走 name2id / 别名表，
#   **且仍要在原文里核对得到**（核对用的是英文别名，原文本来就是英/西/葡语）。
ZH_BRAND_NAMES = {
    "苹果": "Apple", "三星": "Samsung", "荣耀": "Honor", "小米": "Xiaomi", "红米": "Xiaomi",
    "Acme": "Acme", "摩托罗拉": "Motorola", "佳明": "Garmin", "索尼": "Sony",
    "森海塞尔": "Sennheiser", "联想": "Lenovo", "惠普": "HP", "华硕": "ASUS",
    "戴尔": "Dell", "宏碁": "Acer", "宏基": "Acer", "博士": "Bose", "真我": "realme",
    "声阔": "Soundcore", "安克": "Soundcore",
}


def resolve_brand(claimed, name2id: dict[str, int],
                  alias_map: dict[int, list[str]]) -> int | None:
    """模型报的品牌名 → brand_id（不做原文核对，核对由调用方做）。

    接受：英文名 / 别名 / 上表里的中文名；null、空、库里没有的公司名 → None。
    intel._verified_brand 与 brandintel._accept 共用这一个实现。
    """
    c = str(claimed or "").strip()
    if not c or c.lower() == "null":
        return None
    c = ZH_BRAND_NAMES.get(c, c)
    low = c.lower()
    bid = name2id.get(low)
    if bid is None:
        for b, al in alias_map.items():
            if any(low == a.lower() for a in al):
                bid = b
                break
    return bid
...
def _alias_map() -> dict[int, list[str]]:
    ...
    for b in db.q("SELECT id,name,aliases FROM brand"):
        try:
            ...
            raw = []
        # ★ 品牌**本名**无条件保留：len>=3 是为了剔掉短而杂的别名，
        #   但它把 "HP" 一起剔了 —— 一篇只写 HP 不写产品线的新闻永远核对不到，
        #   模型报了 HP 也会被当成"原文无据"丢掉。_hit 是词边界匹配，
        #   "hp" 不会命中 chip / whp，本名进表是安全的。
        al = {str(a).strip() for a in [b["name"], *raw]
              if len(str(a).strip()) >= 3 or str(a).strip() == b["name"]}
        out[b["id"]] = sorted(al)
    return out
